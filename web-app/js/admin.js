...
function appendSubjects(obj) {
    ...
    var counter = 0;
    $("#subjectList").empty();
//    console.log("===="+obj.allSubjects.length)
...
function deleteStudent(studentId) {
    var data = studentId
    window.location.href = '/UniversityProject/student/deleteStudents?studentId=' + data;
}
...
function loadSpeciality(t){
    var panel=$(t).val()
    $.ajax({
        type: "post",
        url: url('patient', 'loadSpeciality', ''),
        data: {panel:panel},
        success: function (data) {
            $('#speciality').empty().append('<option value="">Select Specialist</option> ')

            for (var i=0; i<data.length;i++)
            $('#speciality').append('<option value="'+data[i].id+'">'+data[i].specName+'</option> ')
        }
    })
}


function loadDoctor(t){
    var panel=$('#panel').val()
    var speciality=$(t).val()
    $.ajax({
        type: "post",
        url: url('patient', 'loadDoctor', ''),
        data: {panel:panel,speciality:speciality},
        success: function (data) {
